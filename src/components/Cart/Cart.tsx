...
import { removeCartItem, submitOrder } from 'src/actions';
...
const Cart: React.FC<CartProps> = ({ selectedMenuList, removeCartItem, submitOrder }) => {
  ...
  return (
    <div className="cart">
      ...
      {selectedMenuList.map((selectedMenu, i) => (<SelectedMenuPreview key={selectedMenu.menuName} selectedMenu={selectedMenu} index={i} removeCartItem={removeCartItem}/>))}
    </div>
  );
};
